...
export const LanguagesDropdown = () => {
  ...
  const changeLanguageHandler = (language: 'fr' | 'en') => {
    i18n.changeLanguage(language);
    setIsOpen(false);
  };

  const result = languages.map((language) => {
    const activeLanguage = i18n.language === language.languageName ? '#d3D3D3' : '';
    return (
      <span
        key={language.languageName}
        onClick={() => {
          changeLanguageHandler(language.languageName);
        }}
        style={{ background: activeLanguage }}
        className={`${styles.languages_list_item}`}>
